    ...
    <form action="{{ route('PembantuPPTKView.dokumenjustifikasi.update', ['id' => $dokumenJustifikasi->id]) }}" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
                ...
                <select name="dpa_id" id="dpa_id" class="form-control" required disabled>
                    @foreach($dpas as $dpa)
                        <option value="{{ $dpa->id }}" {{ $dokumenJustifikasi->dpa_id == $dpa->id ? 'selected' : '' }}>{{ $dpa->nomor_dpa }}</option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="form-group">
            ...
            <button type="submit" class="btn btn-primary">Update Dokumen Justifikasi Data</button>
        </form>
    </div>
@endsection
